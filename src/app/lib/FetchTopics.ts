...
export default async function fetchFromPlaylist(playlistId: string) {
  const cacheKey = `Youtubelist:${playlistId}`;
  ...
  try {
    ...
    const url = `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=4&playlistId=${playlistId}&key=${process.env.YOUTUBE_API_KEY}`;
